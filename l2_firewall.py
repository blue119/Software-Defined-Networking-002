...
class LearningSwitch (object):
  ...
  def DeleteRule(self, dpidstr, src = 0):
    try:
      ...
      log.debug("Cannot find rule in %s: %s", dpidstr, src)

  def CheckRule(self, dpidstr, src = 0):
    try:
      entry = self.firewall[(dpidstr, src)]
      if (entry == True):
        log.debug("Rule %s found in %s: FORWARD", dpidstr, src)
      else:
        log.debug("Rule %s found in %s: DROP", dpidstr, src)
      return entry
    except KeyError:
      log.debug("Rule %s NOT found in %s: DROP", dpidstr, src)
    return False
